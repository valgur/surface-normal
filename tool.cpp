...
void Mat2CvMat(Mat *Input, CvMat *out) { *out = *Input; }
void CvMat2Mat(CvMat *Input, Mat *out) { *out = Mat(Input, true); }
...
void get_dir_para(string inputdir, float *fcxcy) {
  ...
  if (data == "2011_09_26") {
    ...
    fcxcy[1] = 596.5593;
    fcxcy[2] = 149.854;
  } else if (data == "2011_09_28") {
    fcxcy[0] = 707.0493;
    fcxcy[1] = 604.0814;
    fcxcy[2] = 162.5066;
  } else if (data == "2011_09_29") {
    fcxcy[0] = 718.3351;
    fcxcy[1] = 600.3891;
    fcxcy[2] = 159.5122;
  } else if (data == "2011_09_30") {
    fcxcy[0] = 707.0912;
    fcxcy[1] = 601.8873;
    fcxcy[2] = 165.1104;
  } else if (data == "2011_10_03") {
    fcxcy[0] = 718.856;
    fcxcy[1] = 607.1928;
    fcxcy[2] = 161.2157;
  } else {
    cout << "Dir data:" << data << " is unexpected. please check." << endl;
    exit(1);
  }
  //
}
  ...
  for (int i = 0; i < rows; i++)
    for (int j = 0; j < cols; j++) {
      if (src.at<unsigned short>(i, j) != 0) {
        res1.at<unsigned short>(i, j) = src.at<unsigned short>(i, j);
        continue;
      }
      int rdx = 0, rdy = 0;
      ...
      for (int m = 0; m < windowsize; m++)
        for (int n = 0; n < windowsize; n++) {
          ...
          if (idx >= rows || idy >= cols || idx < 0 || idy < 0)
            continue;
          if (src.at<unsigned short>(idx, idy) == 0)
            continue;
...
Vec3b Fill(int i, int j, Mat &src, int size) {
  int w = src.cols;
  int h = src.rows;
...
Mat Interpoletion(Mat &src, int size = 8) {
  int cols = src.cols;
  int rows = src.rows;
...
void get_pic_para(string name, float *fcxcy) {
